...
app.post('/api/tasks/:id/approve/:username', auth(ALL_ROLES), async (req, res) => {
  ...
  await db.markAssigneeDone(task.id, req.params.username, req.user.name, 'approve');
...
app.post('/api/tasks/:id/release-stage/:stageNum', auth(ALL_ROLES), async (req, res) => {
  ...
  await db.releaseStage(task.id, stageNum, req.user.name);
...
app.post('/api/tasks/:id/reject/:username', auth(ALL_ROLES), async (req, res) => {
  ...
  await db.resetAssigneeSubmission(task.id, req.params.username);
...
app.post('/api/tasks/:id/close', auth(ALL_ROLES), async (req, res) => {
  ...
  await db.closeTask(task.id, req.user.name);
...
app.post('/api/tasks/:id/cancel', auth(ALL_ROLES), async (req, res) => {
  ...
  await db.cancelTask(task.id, req.user.name, reason);
  releaseDependentsOf(task);
...
async function sendDeadlineReminders() {
  ...
  for (const t of tasks) {
    ...
    if (await db.isTaskBlocked(t.id)) return; // blocked tasks stay exempt from all reminder types
