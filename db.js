...
async function ensureColumn(table, column, decl) {
  const exists = await get('SELECT 1 FROM information_schema.columns WHERE table_name=? AND column_name=?', [table, column]);
  if (!exists) await run(`ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS ${column} ${decl}`);
}
...
module.exports = {
  ...
  // ---- tasks ----
  async createTask({ id, title, description, priority, deadline, created_by, created_by_username, depends_on_task_id, attachment, attachment_name, is_drawing_request, parent_task_id, project, phase }) {
    if (project) await module.exports.addProject(project);
    if (phase) await module.exports.addPhase(phase);
    await run(`INSERT INTO tasks(id,title,description,priority,deadline,status,created_by,created_by_username,depends_on_task_id,attachment,attachment_name,is_drawing_request,parent_task_id,project,phase,created_at)
                VALUES(?,?,?,?,?,'open',?,?,?,?,?,?,?,?,?,?)`,
  ...
  async addTaskAssignee(taskId, username, team, stage, isReleased, individualDeadline) {
    await run('INSERT INTO task_assignees(task_id,username,team,stage,is_released,escalation_baseline_at,individual_deadline) VALUES(?,?,?,?,?,?,?) ON CONFLICT (task_id, username) DO NOTHING',
  ...
  async releaseStage(taskId, stage, releasedByName) {
    await run('UPDATE task_assignees SET is_released=1, released_at=?, released_by=?, escalation_baseline_at=? WHERE task_id=? AND stage=?',
      [new Date().toISOString(), releasedByName || null, new Date().toISOString(), taskId, stage]);
    await module.exports.bumpTaskVersion(taskId);
  },
  async setAutoReleaseStages(taskId, on) { await run('UPDATE tasks SET auto_release_stages=? WHERE id=?', [on ? 1 : 0, taskId]); },
  ...
  async resetAssigneeSubmission(taskId, username) {
    await run('UPDATE task_assignees SET submitted_at=NULL, submission_note=NULL WHERE task_id=? AND username=?', [taskId, username]);
    await module.exports.bumpTaskVersion(taskId);
  },
  async markAssigneeDone(taskId, username, byName, decision) {
    await run('UPDATE task_assignees SET completed_at=?, completed_by=?, decision=? WHERE task_id=? AND username=?',
  ...
  async closeTask(id, closedByName) {
    await run("UPDATE tasks SET status='closed', closed_at=?, closed_by=? WHERE id=?", [new Date().toISOString(), closedByName, id]);
  ...
  async cancelTask(id, cancelledByName, reason) {
    await run("UPDATE tasks SET status='cancelled', cancelled_at=?, cancelled_by=?, cancel_reason=? WHERE id=?", [new Date().toISOString(), cancelledByName, reason || null, id]);
    await module.exports.bumpTaskVersion(id);
  },
  async markAsDrawingRequest(id) { await run('UPDATE tasks SET is_drawing_request=1 WHERE id=?', [id]); },
  async reopenTask(id) {
    await run("UPDATE tasks SET status='open', closed_at=NULL, closed_by=NULL, cancelled_at=NULL, cancelled_by=NULL, cancel_reason=NULL WHERE id=?", [id]);
    await module.exports.bumpTaskVersion(id);
  },
  ...
  async addProject(name) {
    const clean = String(name || '').trim();
    if (!clean) return;
    await run('INSERT INTO projects(name, created_at) VALUES(?, ?) ON CONFLICT (name) DO NOTHING', [clean, new Date().toISOString()]);
  },
  ...
  async addPhase(name) {
    const clean = String(name || '').trim();
    if (!clean) return;
    await run('INSERT INTO task_phases(name, created_at) VALUES(?, ?) ON CONFLICT (name) DO NOTHING', [clean, new Date().toISOString()]);
  },
  ...
  async createNotification({ username, type, message, task_id }) {
    await run('INSERT INTO notifications(username,type,message,task_id,read,created_at) VALUES(?,?,?,?,0,?)',
